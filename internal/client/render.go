...
// renderTree renders the subtree rooted at root into b.
//
...
func renderTree(b *strings.Builder, kids map[string][]core.Node, root core.Node, all bool) {
	writeNode(b, root, 0)
	if all {
		writeChildrenRecursive(b, kids, root.ID, 1)
		return
	}
...
func writeChildrenRecursive(b *strings.Builder, kids map[string][]core.Node, id string, depth int) {
	for _, child := range kids[id] {
		writeNode(b, child, depth)
		writeChildrenRecursive(b, kids, child.ID, depth+1)
	}
}

// writeNode writes a single tree line: indentation, short id, author, and the
// first line of the body.
func writeNode(b *strings.Builder, n core.Node, depth int) {
	indent := strings.Repeat("  ", depth)
	fmt.Fprintf(b, "%s%s  @%s: %s\n", indent, shortID(n.ID), n.AuthorID, firstLine(n.Body))
}
